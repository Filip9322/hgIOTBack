...
async function getAll (req, res) {
  const device_types = await models.Device_Types.findAll(
...
async function getById(req, res) {
  ...
  const device_types = await models.Device_Types.findByPk(id);
...
async function update(req, res) {
  ...
  if(req.body.id === id) {
    await models.Device_Types.update(req.body, {
      where: {
        id: id
      }
    });
...
async function remove(req, res) {
  ...
  if(req.body.id === id) {
    ...
    await models.Device_Types.update(body, {where: {id: id}});
    res.status(200).end();
  } else {
    res.status(400).send(`Bad request: param ID (${id}) does not match body ID (${req.body.id})`);
  }
